...
function Progresion() {
    ...
    const solution = (e) =>{
        ...
        typeData ? setSolutionP(1/3*Math.PI*parseFloat(formObj[0].value)*parseFloat(formObj[0].value)*parseFloat(formObj[1].value)) : setSolutionP(1/3*parseFloat(formObj[0].value)*parseFloat(formObj[1].value));
        setSolutionData([formObj[0].value, formObj[1].value])
    }

    return (
        <section className="training-sect">
                <h2 className="training-name">Онлайн тренажер. Об'єм конуса</h2>
                ...
                <select name="type-colinearity" className="tyme-cor-val" onChange={(e)=>{e.target.value !== "false" ? setTypeData(true) : setTypeData(false)}}>
                  ...
                  <option value={false}>Через площу основи і висоту</option>
                    ...
                    {typeData ? (
                        ...
                        <button className="member-submit">Знайти об'єм конуса</button>
                    </form>
                    <div className="training-solution" id="one-training-solution" style={{display:solutionP!==null ? "inline-block":"none"}}>
                        ...
                        {typeData ? (
                        <p className="training-solution-formule">V = 1 / 3 · π · R<sup>2</sup> · h = π · {solutionData[0]}<sup>2</sup> · {solutionData[1]} = {solutionP} </p>) : (
                        <p className="training-solution-formule">V = 1 / 3 · S<sub>o</sub>h = {solutionData[0]} · {solutionData[1]} = {solutionP} </p>
                        )}
                    </div>
                </div>
                ) : (
                <div className="theory-wrap-cont" id="theory-wrap-cont">
                    <h3 className="theory-text"><span className="theory-text-span">Конус</span> - тіло, отримане об'єднанням всіх лучив, що виходять з однієї точки (вершини конуса) і проходять через плоску поверхню. Також можна сказати, що конус - це тіло, отримане при обертанні прямокутного трикутника навколо одного з його катетів.</h3>
                    <h3 className="theory-text">Формули для обчислення об'єму конуса</h3>
                    <p className="theory-formule">V = 1 / 3 · π · R<sup>2</sup> · h</p>
                    <p className="theory-formule">V = 1 / 3 · S<sub>o</sub> · h</p>
                    <h3 className="theory-text">де V - об'єм конуса,<br/>S<sub>o</sub> - площа основи конуса,<br/>R - радіус основи конуса,<br/>h - висота конуса,<br/>π = 3.141592.</h3>
                </div>
                )}
