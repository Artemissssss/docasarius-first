...
function solveLinearEquations(equations) {
  ...
  if (equations.length < 2) {
    throw new Error("Not enough equations");
  }

  // Get the number of unknowns
  const n = equations[0].length - 1;
  // Create a vector for the unknowns
  let unknowns = new Array(n);

  // Solve the equations one by one
  for (let i = 0; i < n; i++) {
    // Find the equation with the unknown in the current index
    const equation = equations[i];
    // Solve the equation for the unknown
    unknowns[i] = equation[i] / equation[n];
    
    // Substitute the solution into the other equations
    for (let j = 0; j < n; j++) {
      if (j != i) {
        equations[j][i] -= unknowns[i] * equation[j];
      }
    }
  }
  // Return the solution
...
let formData1 = []
let newArrData = new Array(typeColinearityMetr)
for(let i=0; i<formData.length-1; ++i) {
  formData1.push(formData[i].value)
}
for(let i=0; i<typeColinearityMetr; ++i) {
  newArrData[i] = new Array(typeColinearityMetr);
}

for(let i=0; i<typeColinearityMetr; ++i) { //Заполнение матрицы
  for(let j=0; j <= typeColinearityMetr; ++j) {
    newArrData[i][j] = parseFloat(formData[(j+(typeColinearityMetr+1)*i)].value)
  }
}
if(formData1.join("").split("0").join("").split(",").join("") == false){
  setSolutionP([0])
  setSolutionData(formData1)
}else{
  setSolutionP(solveLinearEquations(newArrData))
  setSolutionData(formData1)
}
...
// };
  ...
  return (
    <section className="training-sect">
            <h2 className="training-name">Онлайн тренажер. Розв'язання систем лінійних рівнянь. (Метод підстановки)</h2>
